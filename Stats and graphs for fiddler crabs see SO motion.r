# This is a copy of the R script used to plot the results and perform the statistical analysis for Smithers et al (2024)-
# "Fiddler crabs (Afruca tangeri) detect second-order motion in both intensity and polarization", Communications Biology. 
# For an explanation of the statistical analysis conducted, please refer to the Statistics and Reproducibility section of 
# the methods in the main manuscript.

# Script written by Dr Samuel P. Smithers, Northeastern University, 2022-2024
...
anova(m1_1, Stim, test = "LRT") #Chisq(4)= 62.944, p=6.974e-13 ***
...
#McNemar's chi-squared = 19.531, df = 1, p-value = 9.897e-06 (NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
...
#McNemar's chi-squared = 15.613, df = 1, p-value = 7.772e-05 (NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
max((contingencyTable[3]/contingencyTable[2]), (contingencyTable[2]/contingencyTable[3]))
# OR = 6.75

#Control vs Second-order dynamic texture stimulus
pairwiseData <- subset(I_data, Stimulus == "Control" | Stimulus == 'Second-order dynamic texture stimulus')
pairwiseData <- pairwiseData %>% group_by(CrabID) %>% filter(n()>1) 
Stim_1 <- subset(pairwiseData, Stimulus == "Control")
Stim_1_Res <- Stim_1$BinaryRes
Stim_1_CrabID <- Stim_1$CrabID
Stim_2 <- subset(pairwiseData, Stimulus == "Second-order dynamic texture stimulus")
Stim_2_Res <- Stim_2$BinaryRes
Stim_2_CrabID <- Stim_2$CrabID
table(Stim_1_CrabID == Stim_2_CrabID) #Check that this is TRUE
ResponseTable <- data.frame(Control = Stim_1_Res, Expanding_SO_dynamic_texture = Stim_2_Res)
contingencyTable <- table(ResponseTable) 
contingencyTable
mcnemar.test(contingencyTable, y = NULL, correct = TRUE)
#McNemar's chi-squared = 31.61, df = 1, p-value = 1.885e-08 (NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
max((contingencyTable[3]/contingencyTable[2]), (contingencyTable[2]/contingencyTable[3]))
# OR = 19.5

#Control vs Non-expanding dynamic texture control 
pairwiseData <- subset(I_data, Stimulus == "Control" | Stimulus == 'Non-expanding dynamic texture control ')
pairwiseData <- pairwiseData %>% group_by(CrabID) %>% filter(n()>1) 
Stim_1 <- subset(pairwiseData, Stimulus == "Control")
Stim_1_Res <- Stim_1$BinaryRes
Stim_1_CrabID <- Stim_1$CrabID
Stim_2 <- subset(pairwiseData, Stimulus == "Non-expanding dynamic texture control ")
Stim_2_Res <- Stim_2$BinaryRes
Stim_2_CrabID <- Stim_2$CrabID
table(Stim_1_CrabID == Stim_2_CrabID) #Check that this is TRUE
ResponseTable <- data.frame(Control = Stim_1_Res,  NonExpanding_dynamic_texture= Stim_2_Res)
contingencyTable <- table(ResponseTable) 
contingencyTable
mcnemar.test(contingencyTable, y = NULL, correct = TRUE)
# McNemar's chi-squared = 9.3333, df = 1, p-value = 0.00225 (NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
max((contingencyTable[3]/contingencyTable[2]), (contingencyTable[2]/contingencyTable[3]))
# OR = 6
...
#(NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
max((contingencyTable[3]/contingencyTable[2]), (contingencyTable[2]/contingencyTable[3]))
# OR = 1.272727

#First order vs Second-order dynamic texture stimulus
pairwiseData <- subset(I_data, Stimulus == "First-order stimulus" | Stimulus == 'Second-order dynamic texture stimulus')
pairwiseData <- pairwiseData %>% group_by(CrabID) %>% filter(n()>1) 
Stim_1 <- subset(pairwiseData, Stimulus == "First-order stimulus")
Stim_1_Res <- Stim_1$BinaryRes
Stim_1_CrabID <- Stim_1$CrabID
...
Stim_2_CrabID <- Stim_2$CrabID
table(Stim_1_CrabID == Stim_2_CrabID) #Check that this is TRUE
ResponseTable <- data.frame(FO = Stim_1_Res,  Expanding_SO_dynamic_texture= Stim_2_Res)
contingencyTable <- table(ResponseTable) 
contingencyTable
mcnemar.test(contingencyTable, y = NULL, correct = TRUE)
#McNemar's chi-squared = 4.5, df = 1, p-value = 0.03389 (NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
max((contingencyTable[3]/contingencyTable[2]), (contingencyTable[2]/contingencyTable[3]))
# OR = 3.5

#First order vs Non-expanding dynamic texture control 
pairwiseData <- subset(I_data, Stimulus == "First-order stimulus" | Stimulus == 'Non-expanding dynamic texture control ')
pairwiseData <- pairwiseData %>% group_by(CrabID) %>% filter(n()>1) 
Stim_1 <- subset(pairwiseData, Stimulus == "First-order stimulus")
Stim_1_Res <- Stim_1$BinaryRes
Stim_1_CrabID <- Stim_1$CrabID
...
Stim_2_CrabID <- Stim_2$CrabID
table(Stim_1_CrabID == Stim_2_CrabID) #Check that this is TRUE
ResponseTable <- data.frame(FO = Stim_1_Res,  NonExpanding_dynamic_texture= Stim_2_Res)
contingencyTable <- table(ResponseTable) 
contingencyTable
mcnemar.test(contingencyTable, y = NULL, correct = FALSE)
...
#mean grey vs Second-order dynamic texture stimulus
pairwiseData <- subset(I_data, Stimulus == "Mean grey" | Stimulus == 'Second-order dynamic texture stimulus')
pairwiseData <- pairwiseData %>% group_by(CrabID) %>% filter(n()>1) 
...
Stim_2_CrabID <- Stim_2$CrabID
table(Stim_1_CrabID == Stim_2_CrabID) #Check that this is TRUE
ResponseTable <- data.frame(Midgrey = Stim_1_Res,  Expanding_SO_dynamic_texture = Stim_2_Res)
contingencyTable <- table(ResponseTable) 
contingencyTable
mcnemar.test(contingencyTable, y = NULL, correct = TRUE)
#McNemar's chi-squared = 6.8571, df = 1, p-value = 0.008829 (NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
max((contingencyTable[3]/contingencyTable[2]), (contingencyTable[2]/contingencyTable[3]))
# OR = 4.25

#mean grey vs Non-expanding dynamic texture control 
pairwiseData <- subset(I_data, Stimulus == "Mean grey" | Stimulus == 'Non-expanding dynamic texture control ')
pairwiseData <- pairwiseData %>% group_by(CrabID) %>% filter(n()>1) 
Stim_1 <- subset(pairwiseData, Stimulus == "Mean grey")
Stim_1_Res <- Stim_1$BinaryRes
Stim_1_CrabID <- Stim_1$CrabID
...
Stim_2_CrabID <- Stim_2$CrabID
table(Stim_1_CrabID == Stim_2_CrabID) #Check that this is TRUE
ResponseTable <- data.frame(Midgrey = Stim_1_Res,  NonExpanding_dynamic_texture= Stim_2_Res)
contingencyTable <- table(ResponseTable) 
contingencyTable
mcnemar.test(contingencyTable, y = NULL, correct = FALSE)
...
#Second-order dynamic texture stimulus vs Non-expanding dynamic texture control 
pairwiseData <- subset(I_data, Stimulus == "Second-order dynamic texture stimulus" | Stimulus == 'Non-expanding dynamic texture control ')
pairwiseData <- pairwiseData %>% group_by(CrabID) %>% filter(n()>1) 
Stim_1 <- subset(pairwiseData, Stimulus == "Second-order dynamic texture stimulus")
Stim_1_Res <- Stim_1$BinaryRes
Stim_1_CrabID <- Stim_1$CrabID
Stim_2 <- subset(pairwiseData, Stimulus == "Non-expanding dynamic texture control ")
Stim_2_Res <- Stim_2$BinaryRes
Stim_2_CrabID <- Stim_2$CrabID
table(Stim_1_CrabID == Stim_2_CrabID) #Check that this is TRUE
ResponseTable <- data.frame(Expanding_SO_dynamic_texture = Stim_1_Res,  NonExpanding_dynamic_texture= Stim_2_Res)
contingencyTable <- table(ResponseTable) 
contingencyTable
mcnemar.test(contingencyTable, y = NULL, correct = TRUE)
# McNemar's chi-squared = 15.75, df = 1, p-value = 7.229e-05 (NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
max((contingencyTable[3]/contingencyTable[2]), (contingencyTable[2]/contingencyTable[3]))
# OR = 8.333333
...
# Copy of test results from above showing adjusted P values. The adjusted P value is the one reported in the manuscript.  
...
#Control vs Second-order dynamic texture stimulus: McNemar's chi-squared = 31.61, df = 1, p-value = 1.885e-08, p-adjusted = 1.885e-07***
#Control vs Non-expanding dynamic texture control : McNemar's chi-squared = 9.3333, df = 1, p-value = 0.00225, p-adjusted = 0.0225*
#First order vs mean grey: McNemar's chi-squared = 0.36, df = 1, p-value = 0.5485, p-adjusted = 1 
#First order vs Second-order dynamic texture stimulus: McNemar's chi-squared = 4.5, df = 1, p-value = 0.03389, p-adjusted = 0.3389 
#First order vs Non-expanding dynamic texture control : McNemar's chi-squared = 5.2609, df = 1, p-value = 0.02181, p-adjusted = 0.2181
#mean grey vs Second-order dynamic texture stimulus: McNemar's chi-squared = 6.8571, df = 1, p-value = 0.008829, p-adjusted = 0.08829 
#mean grey vs Non-expanding dynamic texture control : McNemar's chi-squared = 2.6667, df = 1, p-value = 0.1025, p-adjusted = 1
#Second-order dynamic texture stimulus vs Non-expanding dynamic texture control : McNemar's chi-squared = 15.75, df = 1, p-value = 7.229e-05, p-adjusted = 0.0007229** 
...
# with the same model but with the fixed effect of interest removed. 
Stim <-update(m1,~.-Stimulus)
anova(m1_1, Stim, test = "LRT") #Chisq(4)= 45.729, p=2.805e-09 ***
...
#Perform McNemar test
mcnemar.test(contingencyTable, y = NULL, correct = TRUE) # A continuity correction (correct = TRUE) is applied if any of the counts are <5.
#McNemar's chi-squared = 25.037, df = 1, p-value = 5.624e-07 (NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
...
contingencyTable
mcnemar.test(contingencyTable, y = NULL, correct = TRUE)
#McNemar's chi-squared = 4.3478, df = 1, p-value = 0.03706 (NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
max((contingencyTable[3]/contingencyTable[2]), (contingencyTable[2]/contingencyTable[3]))
# OR = 2.833333

#Control vs Second-order dynamic texture stimulus
pairwiseData <- subset(P_data, Stimulus == "Control" | Stimulus == 'Second-order dynamic texture stimulus')
pairwiseData <- pairwiseData %>% group_by(CrabID) %>% filter(n()>1) 
Stim_1 <- subset(pairwiseData, Stimulus == "Control")
Stim_1_Res <- Stim_1$BinaryRes
Stim_1_CrabID <- Stim_1$CrabID
...
contingencyTable
mcnemar.test(contingencyTable, y = NULL, correct = TRUE)
#McNemar's chi-squared = 16, df = 1, p-value = 6.334e-05 (NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
max((contingencyTable[3]/contingencyTable[2]), (contingencyTable[2]/contingencyTable[3]))
# OR = 11.5

#Control vs Non-expanding dynamic texture control 
pairwiseData <- subset(P_data, Stimulus == "Control" | Stimulus == 'Non-expanding dynamic texture control ')
pairwiseData <- pairwiseData %>% group_by(CrabID) %>% filter(n()>1) 
Stim_1 <- subset(pairwiseData, Stimulus == "Control")
Stim_1_Res <- Stim_1$BinaryRes
Stim_1_CrabID <- Stim_1$CrabID
...
contingencyTable
mcnemar.test(contingencyTable, y = NULL, correct = TRUE)
#McNemar's chi-squared = 0.083333, df = 1, p-value = 0.7728 (NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
max((contingencyTable[3]/contingencyTable[2]), (contingencyTable[2]/contingencyTable[3]))
# OR = 1.4
...
#First order vs Second-order dynamic texture stimulus
pairwiseData <- subset(P_data, Stimulus == "First-order stimulus" | Stimulus == 'Second-order dynamic texture stimulus')
pairwiseData <- pairwiseData %>% group_by(CrabID) %>% filter(n()>1) 
Stim_1 <- subset(pairwiseData, Stimulus == "First-order stimulus")
Stim_1_Res <- Stim_1$BinaryRes
Stim_1_CrabID <- Stim_1$CrabID
...
#First order vs Non-expanding dynamic texture control 
pairwiseData <- subset(P_data, Stimulus == "First-order stimulus" | Stimulus == 'Non-expanding dynamic texture control ')
pairwiseData <- pairwiseData %>% group_by(CrabID) %>% filter(n()>1) 
Stim_1 <- subset(pairwiseData, Stimulus == "First-order stimulus")
Stim_1_Res <- Stim_1$BinaryRes
Stim_1_CrabID <- Stim_1$CrabID
...
contingencyTable
mcnemar.test(contingencyTable, y = NULL, correct = TRUE)
#McNemar's chi-squared = 17.455, df = 1, p-value = 2.943e-05 (NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
max((contingencyTable[3]/contingencyTable[2]), (contingencyTable[2]/contingencyTable[3]))
# OR = 7.25

#mean grey vs Second-order dynamic texture stimulus
pairwiseData <- subset(P_data, Stimulus == "Mean grey" | Stimulus == 'Second-order dynamic texture stimulus')
pairwiseData <- pairwiseData %>% group_by(CrabID) %>% filter(n()>1) 
Stim_1 <- subset(pairwiseData, Stimulus == "Mean grey")
Stim_1_Res <- Stim_1$BinaryRes
Stim_1_CrabID <- Stim_1$CrabID
...
#mean grey vs Non-expanding dynamic texture control 
pairwiseData <- subset(P_data, Stimulus == "Mean grey" | Stimulus == 'Non-expanding dynamic texture control ')
pairwiseData <- pairwiseData %>% group_by(CrabID) %>% filter(n()>1) 
Stim_1 <- subset(pairwiseData, Stimulus == "Mean grey")
Stim_1_Res <- Stim_1$BinaryRes
Stim_1_CrabID <- Stim_1$CrabID
...
contingencyTable
mcnemar.test(contingencyTable, y = NULL, correct = TRUE)
#McNemar's chi-squared = 3.3684, df = 1, p-value = 0.06646 (NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
max((contingencyTable[3]/contingencyTable[2]), (contingencyTable[2]/contingencyTable[3]))
# OR = 2.8

#Second-order dynamic texture stimulus vs Non-expanding dynamic texture control 
pairwiseData <- subset(P_data, Stimulus == "Second-order dynamic texture stimulus" | Stimulus == 'Non-expanding dynamic texture control ')
pairwiseData <- pairwiseData %>% group_by(CrabID) %>% filter(n()>1) 
Stim_1 <- subset(pairwiseData, Stimulus == "Second-order dynamic texture stimulus")
Stim_1_Res <- Stim_1$BinaryRes
Stim_1_CrabID <- Stim_1$CrabID
...
contingencyTable
mcnemar.test(contingencyTable, y = NULL, correct = TRUE)
#McNemar's chi-squared = 12, df = 1, p-value = 0.000532 (NOTE: this is the p value BEFORE the Bonferroni correct is applied below and is not the p value reported in the manuscript.)
max((contingencyTable[3]/contingencyTable[2]), (contingencyTable[2]/contingencyTable[3]))
# OR = 5.75
...
#Control vs Second-order dynamic texture stimulus: McNemar's chi-squared = 16, df = 1, p-value = 6.334e-05, p-adjusted = 0.0006334
#Control vs Non-expanding dynamic texture control : McNemar's chi-squared = 0.083333, df = 1, p-value = 0.7728, p-adjusted = 1 
#First order vs mean grey: McNemar's chi-squared = 8, df = 1, p-value = 0.004678, p-adjusted = 0.04678
#First order vs Second-order dynamic texture stimulus: McNemar's chi-squared = 1.6364, df = 1, p-value = 0.2008, p-adjusted = 1
#First order vs Non-expanding dynamic texture control : McNemar's chi-squared = 17.455, df = 1, p-value = 2.943e-05, p-adjusted = 0.0002943
#mean grey vs Second-order dynamic texture stimulus: McNemar's chi-squared = 3.125, df = 1, p-value = 0.0771, p-adjusted = 0.771
#mean grey vs Non-expanding dynamic texture control : McNemar's chi-squared = 3.3684, df = 1, p-value = 0.06646, p-adjusted = 0.6646
#Second-order dynamic texture stimulus vs Non-expanding dynamic texture control : McNemar's chi-squared = 12, df = 1, p-value = 0.000532, p-adjusted = 0.00532
...
I_G <- data.frame(I_CI)
I_G["Stimuli"] <- c("0Control0","1FO1","2MG2","3EDT3","4DTC4")
I_G
...
P_G <- data.frame(P_CI)
P_G["Stimuli"] <- c("0Control0","1FO1","2MG2","3EDT3","4DTC4")
P_G
...
### correctly formatted and labeled outside R.
#############################################################################################################
